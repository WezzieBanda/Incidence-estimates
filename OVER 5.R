...
#####crude incidence
crude_incidence_bydistrict = aggregate(Confirmed/Population ~ data$adm2, data, mean)
...
tm_shape(guinea_prefectures)+tm_borders(col="black", lwd = 2)+tm_polygons(title = "Incidence\n per 1000", col = "CrudeIncidence",breaks=c( 0, 100, 200, 300, 500, 1000, 1500, 2000),
                                                                          palette = c("#4575B4","#74ADD1","#E0F3F8", "#FFFFBF", "#FEE090", "#FDAE61","#F46D43" , "#D73027"),id="NAME_2")+tm_layout(legend.outside = FALSE,legend.position = c("left","bottom"),legend.title.size = 1.0, legend.height=0.45, legend.text.size = 0.8,frame = FALSE,)+
...
#"#4575B4","#74ADD1","#E0F3F8","#FEE090", "#FDAE61","#F46D43","#D73027"
...
alpha_estimate = 0.57 #0.84 #0.475
beta_estimate = 0.62       #0.8#826    #0.378      #0.589
lambdamin_median = 0.5


corrected_incidence_bydistrict = sapply(1:dim(data)[1], function (x) 
  correct_incidence(data$Tested[x] - data$Confirmed[x], data$Confirmed[x], data$Consultation[x] - data$Tested[x], 
...
tm_shape(guinea_prefectures)+tm_borders(col="black", lwd = 2)+tm_polygons(title = "Incidence\n per 1000", col = "CorrectedIncidence",breaks=c(0, 100, 200, 300, 500, 1000, 1500, 2000),
                                                                          palette = c( "#4575B4","#74ADD1","#E0F3F8", "#FFFFBF", "#FEE090", "#FDAE61","#F46D43" , "#D73027"),id="NAME_2")+tm_layout(legend.outside = FALSE,frame = FALSE, legend.show=FALSE)+
